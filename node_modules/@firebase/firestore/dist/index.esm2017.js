...
let PublicBlob = makeConstructorPrivate(Blob, 'Use Blob.fromUint8Array() or Blob.fromBase64String() instead.');
...
function checkForAndReportiOSError(error) {
    ...
    if (iOSVersion >= 12.2 && iOSVersion < 13) {
        ...
        if (error.message.indexOf(IOS_ERROR) >= 0) {
            ...
            // potentially catch.
            setTimeout(() => {
                throw new FirestoreError('internal', `IOS_INDEXEDDB_BUG1: IndexedDb has thrown '${IOS_ERROR}'. This is likely ` +
...
class LruGarbageCollector {
    ...
    runGarbageCollection(txn, activeTargetIds) {
        ...
        let startTs, countedTargetsTs, foundUpperBoundTs, removedTargetsTs, removedDocumentsTs;
        startTs = Date.now();
...
class TransformMutation extends Mutation {
    ...
    get fieldMask() {
        let fieldMask = new SortedSet(FieldPath.comparator);
     ...
     * @return The transform results list.
     */
    localTransformResults(localWriteTime, baseDoc) {
        const transformResults = [];
        for (const fieldTransform of this.fieldTransforms) {
            const transform = fieldTransform.transform;
            let previousValue = null;
            ...
            return this.localDocuments
                ...
                for (const mutation of mutations) {
                    ...
                    if (!mutation.isIdempotent) {
                        ...
                        if (fieldMask) {
                            const baseValues = maybeDoc instanceof Document
 ...
 */
let MIN_SAFE_INTEGER = NumberAsAny.MIN_SAFE_INTEGER || -(Math.pow(2, 53) - 1);
/**
 * Maximum safe integer in Javascript because of floating point precision.
 * Added to not rely on ES6 features.
 */
let MAX_SAFE_INTEGER = NumberAsAny.MAX_SAFE_INTEGER || Math.pow(2, 53) - 1;
/**
 ...
 */
let isInteger = NumberAsAny.isInteger ||
...
class Query {
    ...
    findRelationOpFilter(relationOps) {
        for (const filter of this.filters) {
...
class RelationFilter extends Filter {
    ...
    matches(doc) {
        if (this.field.isKeyField()) {
            ...
            assert(this.op !== RelationOp.ARRAY_CONTAINS &&
                this.op !== RelationOp.ARRAY_CONTAINS_ANY &&
                this.op !== RelationOp.IN, `'${this.op.toString()}' queries don't make sense on document keys.`);
    ...
    matchesValue(other) {
        if (this.op === RelationOp.ARRAY_CONTAINS) {
            ...
            if (this.value instanceof ArrayValue) {
                return (this.value.internalValue.find(element => element.isEqual(other)) !==
                    undefined);
            }
            else {
            ...
            return (other instanceof ArrayValue &&
                other.internalValue.some(lhsElem => {
                    return (this.value instanceof ArrayValue &&
                        this.value.internalValue.find(rhsElem => rhsElem.isEqual(lhsElem)) !== undefined);
                }));
        }
        else {
            // Only compare types with matching backend order (such as double and int).
            return (this.value.typeOrder === other.typeOrder &&
...
class JsonProtoSerializer {
    ...
    toFilter(filters) {
        if (filters.length === 0)
            return;
        const protos = filters.map(filter => filter instanceof RelationFilter
...
class SyncEngine {
     ...
     * snapshot.
     */
    initializeViewAndComputeSnapshot(queryData, current) {
        const query = queryData.query;
     ...
     */
    // PORTING NOTE: Multi-tab only.
    synchronizeViewAndComputeSnapshot(queryView) {
     ...
     */
    runTransaction(updateFunction, retries = 5) {
    ...
    applySuccessfulWrite(mutationBatchResult) {
    ...
    rejectFailedWrite(batchId, error) {
     ...
     */
    // PORTING NOTE: Multi-tab only.
    synchronizeQueryViewsAndRaiseSnapshots(targets) {
        let p = Promise.resolve();
        const activeQueries = [];
        const newViewSnapshots = [];
    ...
    async applyTargetState(targetId, state, error) {
        ...
        if (this.queryViewsByTarget[targetId]) {
            switch (state) {
                case 'current':
                case 'not-current': {
                    return this.localStore.getNewDocumentChanges().then(async (changes) => {
 ...
 * limitations under the License.
 */
const LOG_TAG$9 = 'SharedClientState';
...
class QueryDocumentSnapshot extends DocumentSnapshot {
    constructor(firestore, key, document, fromCache, hasPendingWrites) {
    ...
    where(field, opStr, value) {
        ...
        if (fieldPath.isKeyField()) {
            if (relationOp === RelationOp.ARRAY_CONTAINS ||
                ...
                throw new FirestoreError(Code.INVALID_ARGUMENT, `Invalid Query. You can't perform '${relationOp.toString()}' ` +
            ...
            if (relationOp === RelationOp.IN ||
                ...
                if (value.length > 10) {
                    throw new FirestoreError(Code.INVALID_ARGUMENT, `Invalid Query. '${relationOp.toString()}' filters support a ` +
 ...
 * limitations under the License.
 */
const LOG_TAG$b = 'ConnectivityMonitor';
/**
...
class WebChannelConnection {
    ...
    invokeRPC(rpcName, request, token) {
        ...
        return new Promise((resolve, reject) => {
                ...
                try {
                    switch (xhr.getLastErrorCode()) {
                        ...
                        case ErrorCode.HTTP_ERROR:
                            ...
                            debug(LOG_TAG$c, 'RPC "' + rpcName + '" failed with status:', status, 'response text:', xhr.getResponseText());
                            if (status > 0) {
                                reject(new FirestoreError(mapCodeFromHttpStatus(status), 'Server responded with status ' + xhr.getStatusText()));
                            }
                            else {
            ...
            debug(LOG_TAG$c, 'XHR sending: ', url + ' ' + requestString);
    ...
    openStream(rpcName, token) {
        ...
        debug(LOG_TAG$c, 'Creating WebChannel: ' + url + ' ' + request);
        ...
        unguardedEventListen(WebChannel.EventType.MESSAGE, msg => {
            if (!closed) {
                ...
                if (error) {
                    debug(LOG_TAG$c, 'WebChannel received error:', error);
