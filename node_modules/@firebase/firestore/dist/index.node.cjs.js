...
function checkForAndReportiOSError(error) {
    ...
    if (iOSVersion >= 12.2 && iOSVersion < 13) {
        ...
        if (error.message.indexOf(IOS_ERROR_1) >= 0) {
            ...
            // potentially catch.
            setTimeout(function () {
                throw new FirestoreError('internal', "IOS_INDEXEDDB_BUG1: IndexedDb has thrown '" + IOS_ERROR_1 + "'. This is likely " +
    ...
    RelationFilter.prototype.matchesValue = function (other) {
        ...
        if (this.op === RelationOp.ARRAY_CONTAINS) {
            return (other instanceof ArrayValue &&
                other.internalValue.find(function (element) { return element.isEqual(_this.value); }) !==
                    undefined);
        }
        else if (this.op === RelationOp.IN) {
            if (this.value instanceof ArrayValue) {
                return (this.value.internalValue.find(function (element) { return element.isEqual(other); }) !==
                    undefined);
            }
            else {
            ...
            return (other instanceof ArrayValue &&
                other.internalValue.some(function (lhsElem) {
                    return (_this.value instanceof ArrayValue &&
                        _this.value.internalValue.find(function (rhsElem) {
 ...
 * limitations under the License.
 */
var LOG_TAG$9 = 'SharedClientState';
...
var GrpcConnection = /** @class */ (function () {
    ...
    GrpcConnection.prototype.ensureActiveStub = function () {
        if (!this.cachedStub) {
            debug(LOG_TAG$b, 'Creating Firestore stub.');
    ...
    GrpcConnection.prototype.invokeRPC = function (rpcName, request, token) {
        ...
        return nodePromise(function (callback) {
            debug(LOG_TAG$b, "RPC '" + rpcName + "' invoked with request:", request);
            return stub[rpcName](request, metadata, function (grpcError, value) {
                if (grpcError) {
                    debug(LOG_TAG$b, "RPC '" + rpcName + "' failed with error:", grpcError);
                    callback(new FirestoreError(mapCodeFromRpcCode(grpcError.code), grpcError.message));
                }
                else {
                    debug(LOG_TAG$b, "RPC '" + rpcName + "' completed with response:", value);
    ...
    GrpcConnection.prototype.invokeStreamingRPC = function (rpcName, request, token) {
        ...
        debug(LOG_TAG$b, "RPC '" + rpcName + "' invoked (streaming) with request:", request);
        var stub = this.ensureActiveStub();
        var metadata = createMetadata(this.databaseInfo, token);
        var stream = stub[rpcName](request, metadata);
        stream.on('data', function (response) {
            debug(LOG_TAG$b, "RPC " + rpcName + " received result:", response);
            results.push(response);
        });
        stream.on('end', function () {
            debug(LOG_TAG$b, "RPC '" + rpcName + "' completed.");
    ...
    GrpcConnection.prototype.openStream = function (rpcName, token) {
        ...
        var stream = new StreamBridge({
            sendFn: function (msg) {
                if (!closed) {
                    debug(LOG_TAG$b, 'GRPC stream sending:', msg);
                    try {
                    ...
                    debug(LOG_TAG$b, 'Not sending because gRPC stream is closed:', msg);
                }
            },
            closeFn: function () {
                debug(LOG_TAG$b, 'GRPC stream closed locally via close().');
                close();
            }
        });
        close = function (err) {
        ...
        grpcStream.on('data', function (msg) {
            if (!closed) {
                debug(LOG_TAG$b, 'GRPC stream received:', msg);
                stream.callOnMessage(msg);
            }
        });
        grpcStream.on('end', function () {
            debug(LOG_TAG$b, 'GRPC stream ended.');
            close();
        });
        grpcStream.on('error', function (grpcError) {
            debug(LOG_TAG$b, 'GRPC stream error. Code:', grpcError.code, 'Message:', grpcError.message);
            var code = mapCodeFromRpcCode(grpcError.code);
            close(new FirestoreError(code, grpcError.message));
        });
        debug(LOG_TAG$b, 'Opening GRPC stream');
