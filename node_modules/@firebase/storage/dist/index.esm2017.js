...
function unknown() {
    let message = 'An unknown error occurred, please check the error payload for ' +
        'server response.';
...
function unauthenticated() {
    let message = 'User is not authenticated, please authenticate using Firebase ' +
 ...
 */
class StringData {
    constructor(data, opt_contentType) {
        this.data = data;
...
class NetworkXhrIo {
    constructor() {
        ...
        this.sendPromise_ = make((resolve, reject) => {
     ...
     * @override
     */
    send(url, method, opt_body, opt_headers) {
        if (this.sent_) {
...
class Location {
    ...
    static makeFromUrl(url) {
        ...
        let httpRegex = new RegExp(`^https?://${hostRegex}/${version}/b/${bucketDomain}/o${path}`, 'i');
 ...
 */
class Mapping {
    constructor(server, opt_local, opt_writable, opt_xform) {
        this.server = server;
...
function fromResourceString(authWrapper, resourceString, mappings) {
    let obj = jsonObjectOrNull(resourceString);
    if (obj === null) {
        return null;
    }
    let resource = obj;
...
function downloadUrlFromResourceString(metadata, resourceString) {
    ...
    let tokens = obj['downloadTokens'];
    if (tokens.length === 0) {
        return null;
    }
    let encode = encodeURIComponent;
    let tokensList = tokens.split(',');
    let urls = tokensList.map(function (token) {
        let bucket = metadata['bucket'];
        let path = metadata['fullPath'];
        let urlPart = '/b/' + encode(bucket) + '/o/' + encode(path);
        let base = makeUrl(urlPart);
 ...
 */
class ArgSpec {
    constructor(validator, opt_optional) {
        let self = this;
...
function uploadDataSpec() {
    function validator(p) {
        let valid = p instanceof Uint8Array ||
...
function looseObjectSpec(opt_validator, opt_optional) {
    function validator(p) {
        let isLooseObject = p === null || (isDef(p) && p instanceof Object);
        if (!isLooseObject) {
 ...
 */
class FbsBlob {
    constructor(data, opt_elideCopy) {
        let size = 0;
        let blobType = '';
    ...
    slice(startByte, endByte) {
        if (isNativeBlob(this.data_)) {
            let realBlob = this.data_;
            let sliced = sliceBlob(realBlob, startByte, endByte);
            if (sliced === null) {
                return null;
            }
            ...
            let slice = new Uint8Array(this.data_.buffer, startByte, endByte - startByte);
            return new FbsBlob(slice, true);
        }
    }
    static getBlob(...var_args) {
        if (isNativeBlobDefined()) {
            ...
            let uint8Arrays = var_args.map(function (val) {
                if (isString(val)) {
...
function metadataHandler(authWrapper, mappings) {
    function handler(xhr, text) {
        let metadata = fromResourceString(authWrapper, text, mappings);
...
function getMetadata(authWrapper, location, mappings) {
    ...
    let url = makeUrl(urlPart);
    let method = 'GET';
    let timeout = authWrapper.maxOperationRetryTime();
    let requestInfo = new RequestInfo(url, method, metadataHandler(authWrapper, mappings), timeout);
...
function getDownloadUrl(authWrapper, location, mappings) {
    ...
    let method = 'GET';
    let timeout = authWrapper.maxOperationRetryTime();
    let requestInfo = new RequestInfo(url, method, downloadUrlHandler(authWrapper, mappings), timeout);
    requestInfo.errorHandler = objectErrorHandler(location);
    return requestInfo;
}
function updateMetadata(authWrapper, location, metadata, mappings) {
    ...
    let url = makeUrl(urlPart);
    let method = 'PATCH';
    let body = toResourceString(metadata, mappings);
...
function multipartUpload(authWrapper, location, mappings, blob, opt_metadata) {
    ...
    let metadataString = toResourceString(metadata, mappings);
    let preBlobPart = '--' +
        boundary +
        '\r\n' +
...
function continueResumableUpload(location, authWrapper, url, blob, chunkSize, mappings, opt_status, opt_progressCallback) {
    ...
    let uploadCommand = bytesToUpload === bytesLeft ? 'upload, finalize' : 'upload';
    let headers = {
 ...
 */
class Observer {
    constructor(nextOrObserver, opt_error, opt_complete) {
        let asFunctions = isFunction(nextOrObserver) ||
            isDef(opt_error) ||
            isDef(opt_complete);
        if (asFunctions) {
...
class UploadTask {
    ...
    constructor(ref, authWrapper, location, mappings, blob, metadata = null) {
        ...
        this.promise_ = make((resolve, reject) => {
    ...
    notifyObservers_() {
        this.finishPromise_();
        const observers = clone$1(this.observers_);
...
class Reference {
    ...
    child(childPath) {
        ...
        let newPath = child(this.location.path, childPath);
     ...
     */
    get parent() {
        let newPath = parent(this.location.path);
        if (newPath === null) {
            return null;
        }
        let location = new Location(this.location.bucket, newPath);
     ...
     *     observe the upload.
     */
    putString(string, format = StringFormat.RAW, opt_metadata) {
        validate('putString', [
            stringSpec(),
        ...
        let data = dataFromString(format, string);
        let metadata = clone(opt_metadata);
    ...
    delete() {
        ...
        return this.authWrapper.getAuthToken().then(function (authToken) {
            let requestInfo = deleteObject(self.authWrapper, self.location);
    ...
    list(options) {
        ...
        return this.authWrapper.getAuthToken().then(function (authToken) {
            const op = options || {};
            let requestInfo = list(self.authWrapper, self.location, 
    ...
    getMetadata() {
        ...
        let self = this;
        return this.authWrapper.getAuthToken().then(function (authToken) {
            let requestInfo = getMetadata(self.authWrapper, self.location, self.mappings());
            return self.authWrapper.makeRequest(requestInfo, authToken).getPromise();
        });
    }
    /**
    ...
    updateMetadata(metadata) {
        ...
        let self = this;
        return this.authWrapper.getAuthToken().then(function (authToken) {
            let requestInfo = updateMetadata(self.authWrapper, self.location, metadata, self.mappings());
            return self.authWrapper.makeRequest(requestInfo, authToken).getPromise();
        });
    }
    /**
    ...
    getDownloadURL() {
        ...
        let self = this;
        return this.authWrapper.getAuthToken().then(function (authToken) {
            let requestInfo = getDownloadUrl(self.authWrapper, self.location, self.mappings());
...
class AuthWrapper {
    ...
    static extractBucket_(config) {
        let bucketString = config[CONFIG_STORAGE_BUCKET_KEY] || null;
        if (bucketString == null) {
            return null;
        }
        let loc = Location.makeFromBucketSpec(bucketString);
        return loc.bucket;
    }
    getAuthToken() {
...
function start(f, callback, timeout) {
    ...
    function handler(success, ...var_args) {
        if (triggeredCallback) {
            return;
        }
        if (success) {
...
class NetworkRequest {
    ...
    start_() {
        ...
        function doTheRequest(backoffCallback, canceled) {
            ...
            xhr
                ...
                xhr = xhr;
                let hitServer = xhr.getErrorCode() === ErrorCode.NO_ERROR;
                let status = xhr.getStatus();
                if (!hitServer || self.isRetryStatusCode_(status)) {
                    let wasCanceled = xhr.getErrorCode() === ErrorCode.ABORT;
                    backoffCallback(false, new RequestEndStatus(false, null, wasCanceled));
                    return;
                }
                let successCode = contains$1(self.successCodes_, status);
        ...
        function backoffDone(requestWentThrough, status) {
            let resolve = self.resolve_;
            let reject = self.reject_;
            let xhr = status.xhr;
    ...
    isRetryStatusCode_(status) {
        ...
        let isExtraRetryCode = contains$1(extraRetryCodes, status);
        let isRequestSpecificRetryCode = contains$1(this.additionalRetryCodes_, status);
...
class RequestEndStatus {
    constructor(wasSuccessCode, xhr, opt_canceled) {
...
function makeRequest(requestInfo, authToken, pool) {
    let queryPart = makeQueryString(requestInfo.urlParams);
